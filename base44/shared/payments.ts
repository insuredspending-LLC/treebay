...
export async function processTestPayment(svc, orderId, outcome, actor) {
  ...
  if (outcome === "TEST_SUCCESS") {
    ...
    await svc.entities.PaymentRecord.update(payment.id, {
      status: "paid", paid_at: now, transaction_ref: transactionRef, provider_payment_id: transactionRef,
    });
    ...
    await recordOrderEvent(svc, { order_id: orderId, event_type: "payment_succeeded", actor_type: "payment_provider", actor_id: "trebay_test", description: "Test payment succeeded (" + transactionRef + ")" });
    ...
    order = await svc.entities.Order.get(orderId);
    // The ONE financial allocation — exactly once, only now that money arrived.
    if (cq) await createAllocationLedger(svc, order, cq, transactionRef);
    // ONLY payment exceptions are resolved here.
